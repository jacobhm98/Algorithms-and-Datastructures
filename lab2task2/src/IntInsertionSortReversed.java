...
for the necessary information to fill up an array of integers. The main class also inputs the numbers with a reversed sign
as a way to sort in a reversed order.
 */
...
public class IntInsertionSortReversed {
    ...
    public static void main(String[] args) {     //prompts the user for dimensions + elements that it inputs into an array, then calls method sort with this array as arg
        ...
        for (int i = 0; i < capacity.length; i++) {
            capacity[i] = (scanify.nextInt() * -1); //turns the input into negative numbers so abs value will be sorted descendingly
        }
        System.out.println(Arrays.toString(capacity));
        sort(capacity);
        for (int i = 0; i < capacity.length; i++) {
            capacity[i] *= -1;      //turns the individual elements back into positive numbers so the end result is that the array is sorted descendingly
        }
        System.out.println(Arrays.toString(capacity));
    }
}